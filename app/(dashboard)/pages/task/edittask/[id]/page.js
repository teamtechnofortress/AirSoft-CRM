...
const UpdateTask = ({params}) => {
  ...
  const fetchtaskrecord = useCallback(async () => {
    try {
      const response = await axios.get(`${process.env.NEXT_PUBLIC_HOST}/oldapi/task/gettask/${id}`);
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (id) {
        setLoading(true);
        await fetchtaskrecord();
        await fetchalluser();
        setLoading(false);
      }
    };
    fetchData();
  }, [id, fetchtaskrecord, fetchalluser]);
  ...
  const handleSubmit = async (event) => {  
    ...
    try {
      setSubmitting(true);
      const response = await axios.post(`${process.env.NEXT_PUBLIC_HOST}/oldapi/task/update`, formData);
