...
const MyVerticallyCenteredModal = ({ show, onHide, products, loading, setSelectedProducts, selectedProducts,fetchAllProducts,searchField,setSearchField , setModalShow, setLoading, setProducts, filteredCache, setFilteredCache }) => {
 ...
 useEffect(() => {
  ...
  const handleProductSelect = (product, variation = null) => {
    setTempSelectedProducts((prevSelected) => {
      ...
      const finalProduct = variation
        ? { ...variation, parent_id: product.id, parent_name: product.name }
        : product;
  
      return [...prevSelected, finalProduct];
    });
  };
                                                ...
                                                products.map((product) => (
                                                    <div key={product.id} className="product-container mb-3">
                                                        <div className="d-flex justify-content-between align-items-center mb-2 gap-2 px-3">
                                                            ...
                                                            <Form.Check
                                                              type="checkbox"
                                                              checked={tempSelectedProducts.some((p) => (p?.data?.id || p?.id) === product.id)}
                                                            ...
                                                            <div className="variation-container ms-4">
                                                                {product.variations.map((variation, index) => (
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <Form.Check
                                                                          type="checkbox"
                                                                          checked={tempSelectedProducts.some((p) => (p?.data?.id || p?.id) === variation.id)}
